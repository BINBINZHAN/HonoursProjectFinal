...
raExpFirst(A) ::= RA_P raSelcollist(B) LP raExpSecond(C) RP.{
  A = raCalculateProjectOp(pParse,B,C);
}

raExpFirst(A) ::= RA_S raExpr(B) LP raExpSecond(C) RP.{
...
//
%type raSetOp {int}
raSetOp(A) ::= RA_U.            {A = TK_UNION; }
raSetOp(A) ::= RA_D.           {A = TK_EXCEPT; }
raSetOp(A) ::= RA_I.    {A = TK_INTERSECT; }
...
%type raJoinOp {int}
raJoinOp(X) ::= RA_X.         { X = JT_INNER; }
raJoinOp(X) ::= RA_J.     { X = JT_NATURAL;}
